    ...
    async hardenMatches(
      ...
      try {
        ...
        for (
          ...
          const results = await validateCandidateGroups(
            client,
            candidateGroups,
          );
        ...
        const invalidated = candidateGroups.map(cg => {
          ...
          return {
            ...cg,
            candidateMask: invalidateCandidateMask(cg.candidateMask),
          };
        });

        const fallbackResults = await validateCandidateGroups(
          client,
          invalidated,
        );
        const safeFallbackByKey = new Map(
          fallbackResults
            .filter(
              r => !r.isSupported || r.decision === MASK_SAFETY_DECISIONS.safe,
            )
