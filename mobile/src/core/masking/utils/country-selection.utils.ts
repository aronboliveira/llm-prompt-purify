...
export function summarizeCountrySelection(
  countryProfiles:
    | readonly CountryProfileSummary[]
    | readonly CountryProfileDefinition[],
): string {
  ...
  const preview = countryProfiles
    .slice(0, 3)
    .map(countryProfile => countryProfile.flagEmoji)
    .join(" ");
...
export function getSelectedLanguageFamilies(
  ...
  return Object.freeze(
    Array.from(
      new Set(
        countryProfiles.map(countryProfile => countryProfile.languageFamily),
      ),
    ),
  );
}
