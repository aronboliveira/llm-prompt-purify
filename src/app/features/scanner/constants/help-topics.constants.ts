...
export const HELP_TOPICS: Readonly<Record<HelpTopicId, HelpTopic>> = Object.freeze({
  ...
  country: {
    ...
    paragraphs: [
      ...
      "Global-only mode keeps the browser scanning for reusable patterns such as credentials, emails, payment strings, and shared labeled fields, but it skips country-specific document formats like CPF, CURP, RUT, DNI, Aadhaar, INN, or Chinese resident IDs.",
    ],
  ...
  workflow: {
    id: "workflow",
    paragraphs: [
      "The intended path is now translator-like: set the scope, paste the raw prompt, wait for the short local masking pass, adjust mask controls if needed, then copy the protected version.",
