...
export function applyCandidateGroupsToMatches(
  ...
  const candidateMaskByKey = new Map(
    candidateGroups.map(candidateGroup => [candidateGroup.key, candidateGroup.candidateMask])
  );

  return matches.map(match => {
    const candidateMask = candidateMaskByKey.get(createMaskSafetyGroupKey(match.ruleId, match.value));
...
export function buildValidationCandidates(
  ...
  return candidateGroups.map(candidateGroup => ({
...
export function createMaskSafetyGroupKey(ruleId: string, sourceValue: string): string {
...
export function groupMaskSafetyCandidates(
  ...
  for (const match of matches) {
    if (match.category !== "financial" && match.category !== "identifier") continue;
